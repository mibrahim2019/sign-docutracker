...
Verifying webhook requests ensures that incoming payloads originate from Docutracker and have not been tampered with. Without verification, attackers could forge requests to your endpoint and trigger unintended actions.

## How Docutracker Signs Webhooks

When you configure a webhook with a secret, Docutracker includes that secret in every webhook request via the `X-Docutracker-Secret` header. Your server should compare this header value against your stored secret to authenticate the request.

```bash
POST /your-webhook-endpoint HTTP/1.1
Host: your-server.com
Content-Type: application/json
X-Docutracker-Secret: your_webhook_secret_here
...
| Header               | Description                                             |
| -------------------- | ------------------------------------------------------- |
| `X-Docutracker-Secret` | The secret key you configured when creating the webhook |
...
<Steps>
  <Step>
    Extract the `X-Docutracker-Secret` header from the incoming request
  </Step>
  <Step>
...
app.post('/webhooks/docutracker', (req, res) => {
  const receivedSecret = req.headers['x-docutracker-secret'];
...
@app.route('/webhooks/docutracker', methods=['POST'])
def handle_webhook():
    received_secret = request.headers.get('X-Docutracker-Secret', '')
...
| --------------- | --------------------------------------- | ------------------------------------------------------------------ |
| Secret mismatch | Webhook secret changed or misconfigured | Verify the secret in your environment matches the one in Docutracker |
| Empty header    | Webhook created without a secret        | Add a secret to the webhook configuration in Docutracker             |
...
3. Update the webhook configuration in Docutracker
...
app.post('/webhooks/docutracker', webhookLimiter, (req, res) => {
  // Handle webhook...
});
```
