---
title: Svelte
description: Embed Docutracker signing in your Svelte application using the official SDK.
---
...
<Tabs items={['npm', 'yarn', 'pnpm']}>
  <Tab value="npm">``` npm install @docutracker/embed-svelte ```</Tab>
  <Tab value="yarn">``` yarn add @docutracker/embed-svelte ```</Tab>
  <Tab value="pnpm">``` pnpm add @docutracker/embed-svelte ```</Tab>
</Tabs>
...
```svelte
<script lang="ts">
  import { EmbedDirectTemplate } from '@docutracker/embed-svelte';
...
```svelte
<script lang="ts">
  import { EmbedSignDocument } from '@docutracker/embed-svelte';
