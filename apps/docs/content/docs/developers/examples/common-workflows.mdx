...
app.post('/webhooks/docutracker', (req, res) => {
  ...
  const secret = req.headers['x-docutracker-secret'] as string;
...
Register your endpoint in the Docutracker dashboard:
...
<Steps>
  ...
  <Step>
    Enter your webhook URL (e.g. <code>https://your-app.com/webhooks/docutracker</code>)
  </Step>
  <Step>
...
class DocutrackerApiError extends Error {
  code?: string;
  statusCode: number;
  details?: Record<string, unknown>;
...
try {
  ...
  if (error instanceof DocutrackerApiError) {
...
- [API Reference](https://openapi.docutracker.com/) - Full endpoint documentation
