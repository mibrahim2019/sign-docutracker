...
description: Complete reference for all environment variables used to configure a self-hosted Docutracker instance.
---

## Required Variables

These variables must be set for Docutracker to function:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL of your Docutracker instance (e.g., `https://sign.example.com`)                   |
...
Docutracker requires PostgreSQL 14 or higher.
...
Docutracker supports multiple email transports for sending notifications.
...
Docutracker can store documents in the database or S3-compatible storage.
...
Docutracker requires a certificate to digitally sign documents.
...
Docutracker can use Google Vertex AI for recipient and field detection.
...
Docutracker uses a PostgreSQL-based job queue by default. Jobs (email delivery, document processing, webhook dispatch) are stored in the `BackgroundJob` table and processed via internal HTTP requests. No external queue service like Redis is required.
...
These variables require an active [Enterprise Edition](/docs/policies/enterprise-edition) license. Obtain a license key from [license.docutracker.com](https://license.docutracker.com) and set it below to unlock enterprise features such as SSO, embed authoring, and 21 CFR Part 11 compliance.
...
# Database
NEXT_PRIVATE_DATABASE_URL="postgres://user:password@localhost:5432/docutracker"
NEXT_PRIVATE_DIRECT_DATABASE_URL="postgres://user:password@localhost:5432/docutracker"
