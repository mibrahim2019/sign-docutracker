...
description: Configure PostgreSQL connection strings, pooling, SSL, migrations, and performance tuning for your Docutracker deployment.
---
...
Docutracker requires **PostgreSQL 14 or later**. No other databases are supported.
...
```
postgresql://docutracker:password@localhost:5432/docutracker
```

**Remote server with SSL:**

```
postgresql://docutracker:password@db.example.com:5432/docutracker?sslmode=require
```
...
```
postgresql://docutracker:p%40ss%23word@localhost:5432/docutracker
```

## Environment Variables

Docutracker uses two database connection variables:
...
Docutracker automatically detects common database environment variable formats used by hosting providers:
...
If your hosting provider sets these variables, Docutracker will use them automatically.
...
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@pooler-host:6432/docutracker?pgbouncer=true

# Direct connection (bypasses PgBouncer)
NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://user:password@db-host:5432/docutracker
```
...
Docutracker uses Prisma, which maintains its own connection pool. Configure pool size with connection string parameters:

```
postgresql://user:password@host:5432/docutracker?connection_limit=10&pool_timeout=30
```
...
```
postgresql://user:password@host:5432/docutracker?sslmode=require
```
...
```
postgresql://user:password@host:5432/docutracker?sslmode=verify-full&sslrootcert=/path/to/ca.crt
```
...
docker run -d \
  ...
  -e NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@host:5432/docutracker?sslmode=verify-full&sslrootcert=/etc/ssl/certs/db-ca.crt" \
  docutracker/docutracker:latest
```

## Running Migrations

Database migrations update your schema when upgrading Docutracker.

### Automatic Migrations

When running Docutracker via Docker, migrations run automatically on container startup. No manual intervention is required.
...
<Accordions type="multiple">
  ...
  <Accordion title="Connection timeout during migration">
    ...
    ```bash
    NEXT_PRIVATE_DIRECT_DATABASE_URL="postgresql://user:password@host:5432/docutracker?connect_timeout=60"
    ```
  </Accordion>
</Accordions>
...
<Accordions type="multiple">
  ...
  <Accordion title="Neon">
    ...
    **Configuration:**
    ...
    ```bash
    # Pooled connection
    NEXT_PRIVATE_DATABASE_URL=postgresql://[user]:[password]@[endpoint]-pooler.region.aws.neon.tech/docutracker?sslmode=require

    # Direct connection for migrations
    NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://[user]:[password]@[endpoint].region.aws.neon.tech/docutracker?sslmode=require
    ```
  ...
  <Accordion title="AWS RDS">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://docutracker:[password]@your-instance.region.rds.amazonaws.com:5432/docutracker?sslmode=require
    NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://docutracker:[password]@your-instance.region.rds.amazonaws.com:5432/docutracker?sslmode=require
    ```
  ...
  <Accordion title="Google Cloud SQL">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://docutracker:[password]@/docutracker?host=/cloudsql/[project]:[region]:[instance]
    ```
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://docutracker:[password]@[public-ip]:5432/docutracker?sslmode=require
    ```
  ...
  <Accordion title="Azure Database for PostgreSQL">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://docutracker@[server-name]:[password]@[server-name].postgres.database.azure.com:5432/docutracker?sslmode=require
    ```
  ...
  <Accordion title="DigitalOcean Managed Databases">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://doadmin:[password]@[cluster-host]:25060/docutracker?sslmode=require
    ```
...
```bash
pg_dump -h host -U user -d docutracker -F c -f docutracker_backup.dump
```

**Restore from backup:**

```bash
pg_restore -h host -U user -d docutracker -c docutracker_backup.dump
```
...
Key parameters for Docutracker workloads:
...
Docutracker includes necessary indexes by default. Additional indexes may help for:
...
<Accordions type="multiple">
  ...
  <Accordion title="Authentication failed">
    ...
    Solutions:
    - Reset password with `ALTER USER docutracker WITH PASSWORD 'newpassword';`
  ...
  <Accordion title="Database does not exist">
    ...
    GRANT ALL PRIVILEGES ON DATABASE docutracker TO docutracker;
    ```
  </Accordion>
  <Accordion title="Permission denied">
    ...
    ```sql
    GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO docutracker;
    GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO docutracker;
    ```
  </Accordion>
</Accordions>
