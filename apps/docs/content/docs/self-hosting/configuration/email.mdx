...
Docutracker supports four email transport methods:
...
SMTP is the most common email transport. Docutracker supports two SMTP authentication methods.
...
Generate an API key in the Resend dashboard for Docutracker to use.
...
Docutracker uses React Email templates for all outgoing emails. The templates are located in `packages/email/templates/` and include:
...
- Custom logo replaces the Docutracker logo
...
Configure Docutracker to use the test server:
...
NEXT_PRIVATE_SMTP_FROM_NAME="Docutracker Test"
```
...
```bash
# Docker
docker logs docutracker 2>&1 | grep -i email
...
For MailChannels, configure DKIM directly in Docutracker using the environment variables described above.
