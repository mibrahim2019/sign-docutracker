---
title: Backups
description: Backup strategies for Docutracker data.
---
...
Docutracker stores data in three locations:
...
```bash
pg_dump -h localhost -U docutracker -d docutracker -F c -f docutracker_backup.dump
```
...
```bash
docker compose exec database pg_dump -U docutracker -F c docutracker > docutracker_backup.dump
```
...
```bash
pg_dump -h localhost -U docutracker -d docutracker -F c \
  -f "docutracker_$(date +%Y%m%d_%H%M%S).dump"
```
...
```bash
pg_dump -h localhost -U docutracker -d docutracker -F p -f docutracker_backup.sql
```
...
```bash
pg_dump -h localhost -U docutracker -d docutracker | gzip > docutracker_backup.sql.gz
```
...
```bash
cp /opt/docutracker/cert.p12 /backup/docutracker/cert.p12
```
...
```bash
echo "$NEXT_PRIVATE_SIGNING_LOCAL_FILE_CONTENTS" | base64 -d > /backup/docutracker/cert.p12
```
...
aws s3api put-bucket-versioning \
  --bucket your-docutracker-bucket \
...
```bash
aws s3 sync s3://your-docutracker-bucket s3://your-backup-bucket
# or
aws s3 sync s3://your-docutracker-bucket /backup/docutracker/documents/
```
...
```bash
aws s3 sync s3://your-bucket /backup/docutracker/documents/ \
...
Create a script to back up all Docutracker components:
...
```bash
#!/bin/bash
# docutracker-backup.sh

set -e

# Configuration
BACKUP_DIR="/backup/docutracker"
RETENTION_DAYS=30
...
S3_BUCKET="your-docutracker-bucket"

# Create backup directory
mkdir -p "$BACKUP_DIR/$TIMESTAMP"

echo "Starting Docutracker backup: $TIMESTAMP"
...
if [ -f "/opt/docutracker/cert.p12" ]; then
  echo "Backing up certificate..."
  cp /opt/docutracker/cert.p12 "$BACKUP_DIR/$TIMESTAMP/cert.p12"
fi
...
tar -czf "$BACKUP_DIR/docutracker_$TIMESTAMP.tar.gz" -C "$BACKUP_DIR" "$TIMESTAMP"
rm -rf "$BACKUP_DIR/$TIMESTAMP"

# Calculate checksum
sha256sum "$BACKUP_DIR/docutracker_$TIMESTAMP.tar.gz" > "$BACKUP_DIR/docutracker_$TIMESTAMP.sha256"

# Remove old backups
echo "Removing backups older than $RETENTION_DAYS days..."
find "$BACKUP_DIR" -name "docutracker_*.tar.gz" -mtime +$RETENTION_DAYS -delete
find "$BACKUP_DIR" -name "docutracker_*.sha256" -mtime +$RETENTION_DAYS -delete

echo "Backup completed: docutracker_$TIMESTAMP.tar.gz"
```
...
```bash
#!/bin/bash
# docutracker-backup-docker.sh

set -e

BACKUP_DIR="/backup/docutracker"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
COMPOSE_PROJECT="docutracker-production"
...
docker compose -p "$COMPOSE_PROJECT" exec -T database \
  pg_dump -U docutracker -F c docutracker > "$BACKUP_DIR/database_$TIMESTAMP.dump"

# Certificate backup (copy from container if needed)
docker compose -p "$COMPOSE_PROJECT" cp \
  docutracker:/opt/docutracker/cert.p12 "$BACKUP_DIR/cert_$TIMESTAMP.p12" 2>/dev/null || true
...
```bash
chmod +x /usr/local/bin/docutracker-backup.sh
```
...
0 2 * * * /usr/local/bin/docutracker-backup.sh >> /var/log/docutracker-backup.log 2>&1

# Hourly backup (for high-activity instances)
0 * * * * /usr/local/bin/docutracker-backup-hourly.sh >> /var/log/docutracker-backup.log 2>&1
```

</Tab>
<Tab value="Systemd timer">

Create `/etc/systemd/system/docutracker-backup.service`:

```ini
[Unit]
Description=Docutracker Backup

[Service]
Type=oneshot
ExecStart=/usr/local/bin/docutracker-backup.sh
User=root
```

Create `/etc/systemd/system/docutracker-backup.timer`:

```ini
[Unit]
Description=Daily Docutracker Backup
...
systemctl enable --now docutracker-backup.timer
```
...
```bash
pg_restore -h localhost -U docutracker -d docutracker -c docutracker_backup.dump
```

Docker Compose:

```bash
docker compose exec -T database pg_restore -U docutracker -d docutracker -c < docutracker_backup.dump
```
...
```bash
psql -h localhost -U docutracker -d docutracker < docutracker_backup.sql
```

For compressed backups:

```bash
gunzip -c docutracker_backup.sql.gz | psql -h localhost -U docutracker -d docutracker
```
...
```bash
cp /backup/docutracker/cert.p12 /opt/docutracker/cert.p12
chmod 644 /opt/docutracker/cert.p12
chown 1001:1001 /opt/docutracker/cert.p12
```
...
```bash
aws s3 sync /backup/docutracker/documents/ s3://your-docutracker-bucket
```
...
```bash
docker compose stop docutracker
```
...
```bash
docker compose exec -T database pg_restore -U docutracker -d docutracker -c < backup.dump
```
...
```bash
docker compose cp cert.p12 docutracker:/opt/docutracker/cert.p12
```
...
```bash
docker compose start docutracker
```
...
BACKUP_FILE="$1"
TEST_DB="docutracker_restore_test"
...
```bash
# Check checksum
sha256sum -c docutracker_backup.sha256

# Verify pg_dump format
pg_restore --list docutracker_backup.dump > /dev/null && echo "Backup is valid"
```
...
```bash
aws s3 cp docutracker_backup.tar.gz s3://your-backup-bucket/docutracker/
```
...
```bash
# Encrypt
gpg --symmetric --cipher-algo AES256 docutracker_backup.tar.gz

# Decrypt
gpg --decrypt docutracker_backup.tar.gz.gpg > docutracker_backup.tar.gz
```
