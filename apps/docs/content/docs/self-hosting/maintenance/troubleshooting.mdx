...
description: Diagnose and resolve common issues with your self-hosted Docutracker instance, organized by component.
---
...
```bash
kubectl logs -l app.kubernetes.io/name=docutracker -n docutracker -f
```
...
```bash
# Docker - find errors
docker logs docutracker 2>&1 | grep -i error

# Docker - find warnings
docker logs docutracker 2>&1 | grep -i warn

# Show last 100 lines
docker logs --tail 100 docutracker
```

### Health Checks

Docutracker exposes a health endpoint to verify the application is running:
...
```bash
docker exec docutracker printenv | grep NEXT
```
...
```bash
docker compose exec docutracker printenv | grep NEXT
```
...
```bash
docker inspect docutracker --format='{{.State.ExitCode}}'
```
...
```bash
# Docker
docker stats docutracker

# Kubernetes
kubectl top pod -n docutracker
```
...
<Steps>
  ...
  <Step>
    Change Docutracker's port mapping: `-p 3001:3000`
  </Step>
</Steps>
...
```
Error: password authentication failed for user "docutracker"
```
...
```sql
ALTER USER docutracker WITH PASSWORD 'newpassword';
```
...
```
Error: database "docutracker" does not exist
```
...
```
Error: too many connections for role "docutracker"
```
...
```
postgresql://user:password@host:5432/docutracker?connection_limit=5
```
...
```bash
docker exec docutracker printenv | grep SMTP
```
...
```bash
docker exec docutracker nc -zv smtp.example.com 587
```
...
```bash
docker exec docutracker printenv | grep SIGNING
```
...
```bash
docker exec docutracker ls -la /opt/docutracker/cert.p12
```
...
```
Error: EACCES: permission denied, open '/opt/docutracker/cert.p12'
```
...
```
  ...
  <Step>
    Update the certificate file and restart Docutracker
  </Step>
</Steps>
...
```bash
docker exec docutracker cat /etc/resolv.conf
```
...
- **[GitHub Discussions](https://github.com/docutracker/docutracker/discussions)**
- **[Discord](https://documen.so/discord)**: Join the Docutracker community Discord
- **[GitHub Issues](https://github.com/docutracker/docutracker/issues)**: For confirmed bugs, open an issue
...
For security vulnerabilities, do not open a public issue. Contact security@docutracker.com directly.
...
<Accordions type="multiple">
  <Accordion title="Check application logs for errors">
    Run `docker logs docutracker` or `docker compose logs docutracker` (or `kubectl logs` for
