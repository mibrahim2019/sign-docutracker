...
description: Practical advice for running Docutracker in production.
---
...
<Callout type="error">
  This is the single most common issue when self-hosting Docutracker. Without a signing certificate,
  the application starts normally but document signing fails.
</Callout>

Docutracker does **not** bundle a [signing certificate](/docs/self-hosting/configuration/signing-certificate) in the Docker image. You must provide one yourself using one of two methods:
...
The default certificate path inside the container is `/opt/docutracker/cert.p12`. You can change this with `NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH`.
...
After starting Docutracker, check the certificate status endpoint:
...
Always set a password when creating your `.p12` certificate. When using OpenSSL, set the password during the export step and provide it to Docutracker via `NEXT_PRIVATE_SIGNING_PASSPHRASE`.

---

## Use the Health Check Endpoints

Docutracker exposes two endpoints for monitoring:
...
docker pull docutracker/docutracker:1.8.0

# Risky — may pull breaking changes
docker pull docutracker/docutracker:latest
```

Check the [GitHub releases](https://github.com/docutracker/docutracker/releases) page for available versions.
...
Background jobs work by Docutracker sending HTTP requests to itself. If your reverse proxy or network setup causes issues with the app reaching its own public URL, set the internal URL:
...
NEXT_PRIVATE_UPLOAD_BUCKET=docutracker-documents
...
Docutracker uses a PostgreSQL-based job queue by default. No Redis, no external message broker. The job system uses your existing database to store and process background tasks like email delivery and document processing.

For high-throughput deployments, Docutracker optionally supports [Inngest](https://www.inngest.com/) as an alternative job provider:
...
The Docutracker container runs as a non-root user (UID 1001). If you mount files into the container (certificates, configuration), ensure they're readable:
