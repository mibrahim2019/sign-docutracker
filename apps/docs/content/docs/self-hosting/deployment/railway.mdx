---
title: Railway
description: Deploy Docutracker on Railway with automatic builds from Git, managed PostgreSQL, and built-in networking. Use the one-click template or manual setup.
---
...
<Callout type="warn">
  ...
  [GitHub Discussions](https://github.com/docutracker/docutracker/discussions) for help.
</Callout>
...
Once deployed, Railway provides a URL like `docutracker-production.up.railway.app`.

Click the URL in the Railway dashboard to access your Docutracker instance.
...
If you prefer more control over the setup, deploy Docutracker manually:
...
- Fork or select the [Docutracker repository](https://github.com/docutracker/docutracker)
...
Click on the Docutracker service and go to the "Variables" tab. Add the following variables:
...
Docutracker runs database migrations automatically on startup.
...
- Click on your Docutracker service
...
Railway provides several options for scaling your Docutracker deployment:
...
<Steps>
<Step>
### Open your Docutracker service

In the Railway dashboard, select the Docutracker service.
...
In the Railway dashboard, select your Docutracker service and open "Settings".
...
<Callout type="warn">
  Docutracker stores sessions in the database, so horizontal scaling works without additional
...
Railway uses usage-based pricing. Estimated monthly costs for Docutracker:
...
Docutracker requires a signing certificate to sign documents. On Railway, you can:

### Option 1: Use Generated Certificate (Default)

Docutracker generates a self-signed certificate automatically if none is provided. This works for testing but shows generic certificate information in signed documents.
...
In your Docutracker service variables, set:
...
<a href="https://app.koyeb.com/deploy?type=git&repository=github.com/docutracker/docutracker&branch=main&name=docutracker-app&builder=dockerfile&dockerfile=/docker/Dockerfile">
  <img
