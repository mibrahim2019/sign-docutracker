---
title: Docker
description: Run Docutracker as a standalone Docker container with an external database. Ideal for container platforms like AWS ECS, Google Cloud Run, or Azure Container Instances.
---
...
These variables must be set for Docutracker to start:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL where Docutracker is accessible (e.g., `https://sign.example.com`) | -                             |
...
Docutracker requires two database connection strings:
...
| `NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH`      | Path to signing certificate inside container                   | `/opt/docutracker/cert.p12` |
...
docker run -d \
  ...
  -e NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@db-host:5432/docutracker" \
  ...
  -e NEXT_PRIVATE_SMTP_FROM_NAME="Docutracker" \
...
- **Volume mount** — mount a `.p12` file from the host into the container at `/opt/docutracker/cert.p12` (shown above). This is the simplest approach for small to moderate deployments.
...
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@db-host:5432/docutracker
...
Docutracker provides health check endpoints for monitoring:
...
```bash
docker inspect --format='{{.State.Health.Status}}' docutracker
```

## Updating the Container

To update Docutracker to a new version:
...
The Docutracker container is stateless. All persistent data is stored in the PostgreSQL database.
...
<Accordions type="multiple">
  <Accordion title="Container fails to start">
    Check the container logs: `docker logs docutracker`. Common causes: missing required environment
