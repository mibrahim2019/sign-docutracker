...
description: Deploy Docutracker on a Linux server without Docker using Node.js and systemd.
---
...
Navigate to the `docutracker` folder and create a `.env` file:
...
NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@localhost:5432/docutracker"
NEXT_PRIVATE_DIRECT_DATABASE_URL="postgresql://user:password@localhost:5432/docutracker"
NEXT_PRIVATE_SMTP_FROM_NAME="Docutracker"
NEXT_PRIVATE_SMTP_FROM_ADDRESS="noreply@your-domain.com"
```

<Callout type="info">
  If you use a reverse proxy in front of Docutracker, set `NEXT_PUBLIC_WEBAPP_URL` to the public URL
...
Create a service file to run Docutracker as a background service on Linux:

```ini
# /etc/systemd/system/docutracker.service
...
Type=simple
User=www-data
WorkingDirectory=/var/www/docutracker
EnvironmentFile=/var/www/docutracker/.env
...
```bash
sudo systemctl enable docutracker
sudo systemctl start docutracker
```

## Reverse Proxy with Nginx

A minimal Nginx configuration for proxying to Docutracker:
