...
description: Deploy Docutracker with Docker Compose, including PostgreSQL. This production-ready setup is suitable for most self-hosted deployments.
---
...
```bash
mkdir docutracker && cd docutracker
curl -O https://raw.githubusercontent.com/docutracker/docutracker/release/docker/production/compose.yml
```
...
cd docutracker/docker/production
```
...
services:
  ...
  docutracker:
    ...
    ports:
      - ${PORT:-3000}:${PORT:-3000}
    volumes:
      - /opt/docutracker/cert.p12:/opt/docutracker/cert.p12:ro
...
| `docutracker` | Main application container, waits for database to be healthy |

The Docutracker container waits for the database health check to pass before starting.
...
```bash
# Database (used by both database and docutracker services)
...
# Public URL where Docutracker is accessible
NEXT_PUBLIC_WEBAPP_URL=https://sign.example.com
NEXT_PRIVATE_INTERNAL_WEBAPP_URL=http://localhost:3000

# Database connection (uses Docker service name)
NEXT_PRIVATE_DATABASE_URL=postgresql://docutracker:your-secure-database-password@database:5432/docutracker
...
```bash
sudo mkdir -p /opt/docutracker
sudo cp /path/to/your/cert.p12 /opt/docutracker/cert.p12
sudo chown 1001:1001 /opt/docutracker/cert.p12
sudo chmod 400 /opt/docutracker/cert.p12
```
...
```
NAME                             STATUS              PORTS
docutracker-production-database-1  running (healthy)   5432/tcp
docutracker-production-docutracker-1 running             0.0.0.0:3000->3000/tcp
```
...
```bash
docker compose --env-file .env restart docutracker
```
...
For production, place Docutracker behind a reverse proxy for SSL termination:
...
```bash
# Manual backup
docker compose exec database pg_dump -U docutracker docutracker > backup.sql

# Restore from backup
docker compose exec -T database psql -U docutracker docutracker < backup.sql
```
...
```bash
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@your-db-host:5432/docutracker
NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://user:password@your-db-host:5432/docutracker
```
...
<Accordions type="multiple">
  <Accordion title="Container fails to start">
    Check logs: `docker compose logs docutracker`. Common causes: missing environment variables
  ...
  <Accordion title="Database connection errors">
    ...
    database psql -U docutracker -d docutracker -c "SELECT 1"`.
  </Accordion>
  ...
  <Accordion title="Emails not sending">
    ...
    password, check logs: `docker compose logs docutracker | grep -i smtp`.
  </Accordion>
