...
## How Docutracker Supports Compliance

Docutracker provides features that support compliance with e-signature laws across jurisdictions:
...
Docutracker maintains an audit trail for each document, recording:
...
Docutracker supports compliance with Simple Electronic Signature (SES) requirements. The following are not currently provided:
