---
title: How to Self-Host Docutracker
description: Learn how to self-host Docutracker on your server or cloud infrastructure using Docker or other deployment methods.
---
...
The following guide will walk you through setting up Docutracker manually on your server or cloud infrastructure.
...
Navigate to the `docutracker` folder and create a `.env` file from the example `.env.example` file:
...
<Callout type="info">
  If you use a reverse proxy in front of Docutracker, don't forget to provide the public URL for the
...
The following guide will walk you through setting up Docutracker using Docker. You can choose between a Docker Compose production setup or a standalone container.

We provide a Docker container for Docutracker, published on both DockerHub and GitHub Container Registry.

- [DockerHub](https://hub.docker.com/r/docutracker/docutracker)
- [GitHub Container Registry](https://ghcr.io/docutracker/docutracker)
...
This setup includes a PostgreSQL database and the Docutracker application. You will need to provide your own SMTP details using environment variables.

<Steps>

### Download the Docker Compose File

Download the Docker Compose file from the Docutracker repository - [compose.yml](https://raw.githubusercontent.com/docutracker/docutracker/release/docker/production/compose.yml).
...
The command will start the PostgreSQL database and the Docutracker application containers.

### Access the Application

Access the Docutracker application by visiting `http://localhost:3000` in your web browser.

</Steps>

### Option 2: Standalone Docker Container

If you prefer to host the Docutracker application on a specific container provider, use the pre-built Docker image from DockerHub or GitHub's Package Registry. You will need to provide your own database and SMTP host.

<Steps>

### Pull the Docker Image

Pull the Docutracker Docker image from DockerHub:
...
docker run -d \
 ...
 -v /path/to/your/keyfile.p12:/opt/docutracker/cert.p12
...
You can access the Docutracker application by visiting the URL you provided for the `NEXT_PUBLIC_WEBAPP_URL` environment variable in your web browser.

</Steps>

### Advanced Configuration

The environment variables listed above are a subset of those available for configuring Docutracker. The table below provides a complete list of environment variables and their descriptions.

For AI setup specifics, see the [AI Recipient & Field Detection (Self-hosting)](./ai-features) page.

| Variable                                     | Description                                                                                                                                             |
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PORT`                                       | The port on which the Docutracker application runs. It defaults to `3000`.                                                                              |
...
| `NEXT_PUBLIC_SUPPORT_EMAIL`                  | The support email address displayed to users (default `support@docutracker.com`).                                                                       |
...
Docutracker collects anonymous telemetry data to help us understand how the software is being used and improve the product. This telemetry is **enabled by default** for self-hosted instances.

### What We Collect

We collect minimal, privacy-preserving data:

- **App Version**: The version of Docutracker you are running
...
We offer several alternative deployment methods for Docutracker if you need more options.
